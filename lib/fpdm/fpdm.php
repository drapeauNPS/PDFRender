...
/*
 * Several modifications were made to this library:
 * 1. The filters and regexps arrays were declared as global so that the library could work
 * within Drupal.
 * 2. Error Handling - I prevented exceptions from being thrown by this library (no longer 'dies').
 * 3. Added some formatting for large bodies of text so that it displays more nicely when
 * merged onto a PDF.
 */
				...
				if($count_entries) {
					...
					if($count_fields) {
						foreach($fields as $name => $value) {
							// If there's a large body of text, format it
							// so a substantial amount fits into the form field.
							$length = strlen($value);
							if($length >= 65) {
								$tmp = wordwrap($value, 65);
								if($length > 300) {
									// Add an ellipsis if the entire body of text can't fit.
									$tmp = substr($tmp, 0, 430);
									$tmp .= '...';
								}
								$value = $tmp;
							}
		...
		**/
		...
		//------------------------
			...
			if($xLen == $oLen) { //...to rectify xref entries 
				...
				$this->CustomError('This file is in an invalid format.  We are unable to render project data on it.');
				//return drupal_set_message(t('This file is in an invalid format.  We are unable to render project data on it.'));
			}
